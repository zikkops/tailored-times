...
template's order form (Birthday 2 family tree, Menu letters, Wedding icons,
field type corrections…). **Those need the Formidable export (P0.2) first**: the
per-template `form_schema` does not exist yet, so there is nothing to correct.

### Done (24 Sep)

- **R2** Wedding 1 blurb: "This Christian wedding" → "This wedding".
- **R4** Home step 1 now lists every template by name, one each, straight from the
  template list (so it can never drift again).
- **R5** Cover page dropped from home step 2 and from the order form: hard copy or
  digital only. `Format` and `calculatePrice` still handle "Cover page" so old
  orders keep their price; the price test covers all three.
- **R8** "What if?" → "Customize your Experience!".
- **R9** One placeholder replaced with the owner's real review (Maya, Birthday Bestie).
- **R11** Step 2 of the order ends with "Anything else we should pay attention to?".
- **R13, R24, R37, R48** Consistent names: Wedding 1, Wedding 2, Sports Tribute,
  Mother's / Father's Day (no more "Christian"/"Muslim"/"Basketball" variants).
- **R14** Text boxes grow as you type and take line breaks.
- **R15** Pages help text says five and above; **8 pages or more forces "designer
  needed" on** (locked, with a note) and `createOrder` refuses 8+ pages without a
  designer, so an edited request cannot get around it. Owner said "more than 8" in
  chat and "more than 4 / five and above" in the sheet: with page options 1, 4, 8,
  12… both mean 8 and up.
- **R90** Spam: honeypot already there; added a per-visitor rate limit (5 orders /
  3 messages per 10 minutes, in memory).

Owner's separate asks the same day: gentler hero drift (covers never reach the
...
- **R12** Numbered template previews after "Customize": we do not have the numbered
  artwork.
- **R16–R88** All per-template field corrections: need the Formidable export (P0.2).
- **R76** "Click here" on the promotion page goes to the retirement template: that is
  the live WordPress site; the rebuild has no such link.
