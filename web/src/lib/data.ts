...
// Step 2 of the order. The real per-template fields still have to be read off
// the live Formidable forms (UPGRADE.md P0.2); until a template has its own
// form_schema, every template uses these.
...
const seedTemplates = (): Template[] =>
  TEMPLATES.map((t) => ({ id: null, slug: t.slug, name: t.name, category: t.category, blurb: t.blurb, previewImages: t.previewImages, formSchema: DEFAULT_FORM_SCHEMA }));
...
const fromRow = (r: TemplateRow): Template => ({
  ...
  formSchema: r.form_schema?.length ? r.form_schema : DEFAULT_FORM_SCHEMA,
});
